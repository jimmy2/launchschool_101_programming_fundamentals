# 101-109 - Small Problems > Medium 1 > 1000 Lights
...
def lights_on(int)
  array = (1..int).to_a
  array = array.map { | value | value == false }
  number, max = 1, array.size
  while number <= max
    array.each_with_index do | value, index |
      array[index] = switch?(value) if ((index + 1) % number) == 0
    end
    number += 1
  end
  output = []
  array.each_with_index do | value, index | 
    output << index + 1 if value == true
  end
  output
end
